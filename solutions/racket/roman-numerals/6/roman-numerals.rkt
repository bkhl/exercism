...
(define (to-roman input)
  (define (convert-partial high mid low input)
    (define (process-additive value digits input)
      (let-values
        ([(count leftover) (quotient/remainder input value)])
        (values (append (build-list count (const value)) digits)
                leftover)))
    ...
    (define (to-string digits)
      (list->string (map (curry dict-ref numerals)
                         (reverse digits))))
    (let*-values
      ([(digits leftover) (process-additive high '() input)]
       [(digits leftover) (process-subtractive high low digits leftover)]
       [(digits leftover) (process-additive mid digits leftover)]
       [(digits leftover) (process-subtractive mid low digits leftover)]
       [(digits leftover) (process-additive low digits leftover)])
